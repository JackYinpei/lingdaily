...
- `primeOutputAudio()` 必须在任何 `fetch`/`await` 之前同步执行，否则首段 AI 音频
  会被浏览器 autoplay 策略吞掉。
- `playbackEpoch` 使打断/断开前排队的异步音频工作失效。
...
Live 声明了 `record_unfamiliar_learning_items` tool。Gemini 触发后先立即回复
`accepted`，再异步调 `/api/learning/items`，避免数据库延迟阻塞实时会话。旧 tool 名
`extract_unfamiliar_english` 仍兼容。
...
- 进度页不有独立表；`/api/learning/progress` 实时扫描当前用户的
  `chat_history`，按请求时区计算活跃天、streak、用户轮次与来源分布。
